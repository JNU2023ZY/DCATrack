    ...
    def forward(self, x_t, x_ot, x_s):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i>3 and i%2==1 and i<11:
                ...
                _, _, mid_x_s = torch.split(mid_x, [H_t * W_t, H_t * W_t, H_s * W_s], dim=1)
        ...
        return x_t_2d, x_ot_2d, x_s_2d, distill_feat_list, early_search_tokens
...
class MixFormer(nn.Module):
    ...
    def __init__(self, backbone, box_head, head_type="CORNER", d_model=768, nhead=8, dim_feedforward=768, dropout=0.1,
                 activation="relu", normalize_before=False, divide_norm=False):
    ...
    def forward(self, template, online_template, search, softmax, run_score_head=True, gt_bboxes=None):
        ...
        template, online_template, search, distill_feat_list, early_search_tokens = self.backbone(template, online_template, search)
        ...
        out = self.forward_head(template, search, softmax=softmax)
    ...
    def forward_head(self, template, search, softmax):
        """
