...
class MixFormer(nn.Module):
    ...
    def forward(self, template, online_template, search, softmax, run_score_head=False, gt_bboxes=None):
        # search: (b, c, h, w)
        if template.dim() == 5:
...
def build_mixformer_vit_adpt_online(cfg, settings=None, train=True) -> MixFormer:
