...
#                early_scores.append(score)
        ...
        return x_t_2d, x_ot_2d, x_s_2d,reg_tokens, distill_feat_list, early_search_tokens, early_reg_tokens
...
class MixFormer(nn.Module):
    ...
    def forward(self, template, online_template, search, softmax, run_score_head=True, gt_bboxes=None):
        ...
        template, online_template, search, reg_tokens, distill_feat_list, early_search_tokens, early_reg_tokens = self.backbone(template, online_template, search)
...
#        early_boxes = [early_box_4, early_box_6, early_box_8, early_box_10]
    ...
    def forward_test(self, template, online_template, search, softmax, run_score_head=True, gt_bboxes=None):
        ...
        if search.dim() == 5:
            search = search.squeeze(0)
        template, online_template, search, flag = self.backbone.forward_test(template, online_template, search, self.mid_score_head)
        # Forward the corner head and score head
        out_dict = {}
        if flag == 4:
