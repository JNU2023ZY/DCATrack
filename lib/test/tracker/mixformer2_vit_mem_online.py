...
import cv2
import os
from lib.models.mixformer2_vit import build_mixformer2_vit_mem_online
...
class MixFormerOnline(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        network = build_mixformer2_vit_mem_online(params.cfg, train=False)
...
#        save_path = os.path.join("/data_F/zhouyong/MixFormerV2-main/Mem_VIS/search", "%04d.jpg" % self.frame_id)
...
#        print(self.frame_id)
        with torch.no_grad():
            ...
            out_dict, mem, z = self.network.forward_test(self.template, self.online_template, search, self.mem, self.z,
                                                         softmax=True, run_score_head=True, frame=self.frame_id)
        ...
        #            print(self.frame_id)
        #            print(pred_score)
        #            save_path = os.path.join("/data_F/zhouyong/MixFormerV2-main/DEBUG_VIS/low_score_search", "%04d.jpg" % self.frame_id)
        #            x_patch_arr = cv2.cvtColor(x_patch_arr, cv2.COLOR_BGR2RGB)
        #            cv2.imwrite(save_path, x_patch_arr)
        #            with open("/data_F/zhouyong/MixFormerV2-main/DEBUG_VIS/low_score_search/low_score_id.txt", 'a') as fi:
        #                fi.write(f"{self.frame_id}: {pred_score}  |")

        if pred_score > 0.75 and pred_score > self.max_pred_score:  # pred_score > 0.5
        ...
        if self.frame_id % self.update_interval == 0:
            ...
            #           print(f"frame{self.frame_id}: update score{self.max_pred_score}")
            #           save_path = os.path.join("/data_F/zhouyong/MixFormerV2-main/DEBUG_VIS/update_online", "%04d.jpg" % self.frame_id)
            #           x_patch_arr = cv2.cvtColor(self.max_online_template_arr, cv2.COLOR_BGR2RGB)
            #           cv2.imwrite(save_path, x_patch_arr)
