...
def transform_trackingnet(tracker_name, cfg_name):
    ...
    for item in items:
        ...
        its = os.listdir(src_path)
        print(its)
        for it in its:
            if "all" in it:
                continue
            if "time" not in it:
                src_path_txt = os.path.join(src_path, it)
                dest_path_txt = os.path.join(dest_dir, it)
                bbox_arr = np.loadtxt(src_path_txt, dtype=np.int64, delimiter=',')
                np.savetxt(dest_path_txt, bbox_arr, fmt='%d', delimiter=',')
    # make zip archive
