...
class MixFormerMemActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        # compute losses
        loss, status = self.compute_losses(out_dict, gt_bboxes, labels=labels, early_out_dict=early_out_dict, early_scores=early_scores)
    ...
    def forward_pass(self, data, run_score_head):
        ...
        out_dict, early_out_dict, early_scores = self.net(data['template_images'][0], data['template_images'][1], data['search_images'],
                               softmax=True,
                               run_score_head=run_score_head, gt_bboxes=search_bboxes,addmem=True)
    ...
    def compute_losses(self, pred_dict, gt_bbox, return_status=True, labels=None, early_out_dict=None, early_scores=None):
        ...
        for i in range(len(pred_dict)):
            ...
            # weighted sum
            loss_ = 1.25*(self.loss_weight['ciou'] * ciou_loss + self.loss_weight['l1'] * l1_loss)
            ...
            if ('pred_scores' in pred_dict[i]) and (early_out_dict[i] is not None):
                ...
                for k, early_pred in enumerate(early_out_dict[i]):
                    ...
                    early_ious.append(early_iou)
                    mid_loss += self.objective['mid_score'](early_scores[i][k].unsqueeze(-1), early_iou.unsqueeze(-1))

                #loss = score_loss * self.loss_weight['score'] + mid_loss * self.loss_weight['score']
                score_loss = self.objective['mid_score'](pred_dict[i]['pred_scores'].unsqueeze(-1), iou.unsqueeze(-1))
                loss_ = mid_loss + score_loss
            total_loss += loss_
...
#            early_mean_iou2 = [all_early_ious[1][i].detach().mean() for i in range(len(all_early_ious[1]))]
            if ('pred_scores' in pred_dict[0]) and (early_out_dict[0] is not None):
