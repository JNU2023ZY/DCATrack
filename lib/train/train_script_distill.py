...
from lib.models.mixformer2_vit import build_mixformer2_vit, build_mixformer2_vit_stu, build_mixformer2_vit_online, build_mixformer2_vit_mem
from lib.models.Mambaformer import build_mamba_vision
# forward propagation related
from lib.train.actors import MixFormerDistillStage1Actor, MixFormerDistillStage2Actor,MixFormerMemDistillActor
# for import modules
import importlib


def build_network(script_name, cfg, teacher: bool=False, settings=None):
    train = (not teacher)
    # Create network
    ...
    elif script_name == "mixformer2_vit_online":
        net = build_mixformer2_vit_online(cfg, train=train, settings=settings)
    elif script_name == "mixformer2_vit_mem":
        net = build_mixformer2_vit_mem(cfg, train=train, settings=settings)
    elif script_name == "mixmamba":
        net = build_mamba_vision(cfg, train=train)
    else:
...
def run(settings):
    ...
    net = build_network(settings.script_name, cfg, settings=settings)
    net_teacher = build_network(settings.script_teacher, cfg_teacher,teacher=True,settings=settings)
    ...
    if settings.script_name == 'mixformer2_vit': 
        ...
        objective = {'ciou': ciou_loss, 'l1': l1_loss, 'score': BCEWithLogitsLoss()}
        loss_weight = {'ciou': cfg.TRAIN.IOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'score': cfg.TRAIN.SCORE_WEIGHT}
        actor = MixFormerMemDistillActor(net=net, net_teacher=net_teacher, objective=objective, loss_weight=loss_weight, settings=settings, run_score_head=False,
                                      z_size_teacher=cfg_teacher.DATA.TEMPLATE.SIZE, x_size_teacher=cfg_teacher.DATA.SEARCH.SIZE, feat_sz=cfg.MODEL.FEAT_SZ)
