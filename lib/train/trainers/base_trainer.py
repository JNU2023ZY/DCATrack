        ...
        for i in range(num_tries):
            try:
                ...
                if distill:
                    ...
                    #directory_teacher = '/data_F/zhouyong/MixFormerV2-main/288_qkv_r_distill_t_4_12_300_online_score_6/checkpoints/train/mixformer2_vit_online/288_depth12_score/MixFormer_ep0040.pth.tar'
                    directory_teacher = '/data_F/zhouyong/MixFormerV2-main/MixformerV2_model/mixformerv2_base.pth.tar'
...
#        checkpoint_dict['optimizer']['param_groups'][3]['initial_lr'] = checkpoint_dict['optimizer']['param_groups'][3]['initial_lr'] / 5
#        checkpoint_dict['optimizer']['param_groups'][3]['lr'] = checkpoint_dict['optimizer']['param_groups'][3]['lr'] / 5
    ...
    def load_state_dict(self, checkpoint=None, distill=False):
        ...
        #checkpoint_path = "/data/kangze/MixFormerV2/lib/train/checkpoints/train/mixformer_vit/teacher_mixvit_b/mixformer_vit_base_online.pth.tar"
        checkpoint_dict = torch.load(checkpoint_path, map_location='cpu', weights_only=False)
        ckpt_net = checkpoint_dict["net"]
        #ckpt_net["box_head.layers.0.0.weight"] = ckpt_net["box_head.layers_l.0.0.weight"]
        #ckpt_net["box_head.layers.0.0.bias"] = ckpt_net["box_head.layers_l.0.0.bias"]
        #ckpt_net["box_head.layers.0.1.weight"] = ckpt_net["box_head.layers_l.0.1.weight"]
        #ckpt_net["box_head.layers.0.1.bias"] = ckpt_net["box_head.layers_l.0.1.bias"]
        #ckpt_net["box_head.layers.1.0.weight"] = ckpt_net["box_head.layers_l.1.0.weight"]
        #ckpt_net["box_head.layers.1.0.bias"] = ckpt_net["box_head.layers_l.1.0.bias"]
        #ckpt_net["box_head.layers.1.1.weight"] = ckpt_net["box_head.layers_l.1.1.weight"]
        #ckpt_net["box_head.layers.1.1.bias"] = ckpt_net["box_head.layers_l.1.1.bias"]

        if not distill:
            assert net_type == checkpoint_dict['net_type'], 'Network is not of correct type.'

        missing_k, unexpected_k = net.load_state_dict(ckpt_net, strict=False)
        if is_main_process():
